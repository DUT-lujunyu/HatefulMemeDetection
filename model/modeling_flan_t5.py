...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None, patch_size=None):
        ...
        self.start_ca_layer = 12
        if patch_size is not None:
            self.image_dense = nn.Linear(patch_size, config.d_model)
            # nn.MultiheadAttention:    q: text_dim,   k, v: image_dim
            ...
            # 2024.04.24 在后 num_ca_layers 的 transformer 层之间添加交叉注意力层引入图像信息
            # self.start_ca_layer = config.start_ca_layer
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            ######################
            # 2024.04.24
            if image_ids is not None and self.is_decoder == False:
                if i >= self.start_ca_layer:
        ...
        if image_ids is not None:
            image_embedding = self.image_dense(image_ids)
            # print(image_embedding.shape)  # vit-base [batch_size, patch_num, img_dim]  [32, 197, 768]
            ...
            # hidden_states = hidden_states + image_att
...
class T5ForMultimodalGeneration(T5ForConditionalGeneration):
    ...
    def __init__(self, config: T5Config, patch_size=None):
        super().__init__(config)
        ...
        if not patch_size:
            ...
            self.encoder = JointEncoder(
                encoder_config, self.shared, patch_size)
    ...
    def forward(
        ...
        # print(encoder_outputs[0].shape)  # [batch_size, text_length, text_dim]  [64, 32, 768]
